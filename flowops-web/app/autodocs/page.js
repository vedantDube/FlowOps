...
const DOC_TYPES = [
  ...
  { value: 'knowledge-base', label: 'Knowledge Base', icon: Globe, desc: 'Team knowledge article', color: 'text-amber-500 bg-amber-500/10 border-amber-500/20' },
]
...
export default function AutoDocsPage() {
  ...
  const handleFetchAndUseContent = async () => {
    ...
    try {
      ...
      alert('Failed to fetch file contents: ' + (e.response?.data?.error || e.message))
    } finally {
          ...
          <Card className="mb-6 relative overflow-hidden">
            ...
            <CardHeader className="p-5 pb-3 pt-6">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-semibold text-foreground">New Documentation</p>
              </div>
            </CardHeader>
                  ...
                  >
                    <div
                      className={cn(
                        ...
                        form.type === value ? color : 'bg-muted text-muted-foreground border-transparent',
                      )}
                    >
                      <Icon size={14} />
                    </div>
                ...
                <>
                    ...
                    <div className="mb-4 border border-amber-500/30 bg-amber-500/5 rounded-xl p-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <p className="text-sm font-semibold text-foreground">Selected files are empty</p>
                          <p className="text-xs text-muted-foreground mt-1">
                            The files you selected from {selectedRepo?.fullName || 'the repository'} have no content.
                            To generate docs for specific code, please select files that contain code.
                          </p>
                          <div className="flex items-center gap-3 mt-3">
                            ...
                            >
                              {isGenerating ? (
                                <>
                                  <RefreshCw size={12} className="mr-1.5 animate-spin" /> Generating…
                                </>
                              ) : (
                                <>
                                  <Sparkles size={12} className="mr-1.5" /> Generate General README Instead
                                </>
                              )}
                            </Button>
                            <Button
                  ...
                  <Textarea
                    ...
                    onChange={(e) => {
                      ...
                      if (emptyFileWarning && e.target.value.trim()) setEmptyFileWarning(false)
                    }}
                  ...
                  <div className="flex gap-3">
                    <Button onClick={handleGenerate} disabled={isGenerating || !form.context.trim()}>
                      {isGenerating ? (
                        <>
                    ...
                    <>
                      ...
                      <div className="flex gap-3">
                        ...
                        >
                          {fetchingContent ? (
                            <>
                              <Loader2 size={14} className="mr-2 animate-spin" /> Fetching files…
                            </>
                          ) : (
                            <>
                              <FolderGit2 size={14} className="mr-2" /> Fetch {selectedFiles.size} file
          ...
          <Card className="xl:col-span-3 overflow-hidden">
            {!selectedDoc ? (
              <div className="flex flex-col items-center justify-center h-full min-h-[400px] text-muted-foreground">
                ...
                <p className="text-sm">Choose a document from the sidebar to preview.</p>
              </div>
            ) : (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5 capitalize">
                          {selectedDoc.type?.replace('-', ' ')} · Version {selectedDoc.version} ·{' '}
                          {new Date(selectedDoc.updatedAt).toLocaleDateString()}
                        </p>
                      </div>
                    </div>
                    <Badge
                      variant={selectedDoc.status === 'published' ? 'success' : 'secondary'}
