...
import { Activity, Calendar, Clock, GitMerge, TrendingUp, Users } from 'lucide-react'
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    setIsFetching(true)
    Promise.allSettled([
      fetchPRCycleTime({ orgId }),
      fetchReviewLatency({ orgId }),
  ...
  const dailyAvg = commitData.length > 0 ? (totalCommits / commitData.length).toFixed(1) : '0'
      ...
      <div className="p-6 lg:p-8 max-w-[1440px] mx-auto">
          ...
          {isFetching ? (
            ...
            <>
              <MetricCard
                ...
                color="green"
                trend={-12}
                trendLabel={`vs last ${days === 0 ? 'all' : days + ' days'}`}
              />
              <MetricCard
                ...
                color="teal"
                trend={-8}
                trendLabel={`${reviewLatency.total || 0} reviews total`}
              />
              <MetricCard
              ...
              <MetricCard
                ...
                icon={<Users size={16} />}
                color="yellow"
                subtitle={contributors[0] ? `${contributors[0].commits} commits` : 'No data yet'}
              />
            </>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-6">
          <Card className="overflow-hidden">
            <CardHeader className="p-5 pb-0 flex flex-row items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">{days === 0 ? 'All time' : `Last ${days} days`}</p>
              </div>
          ...
          <Card className="overflow-hidden">
            <CardHeader className="p-5 pb-0 flex flex-row items-center justify-between">
              <div>
                <p className="text-sm font-semibold text-foreground">Code Churn</p>
                <p className="text-xs text-muted-foreground mt-0.5">Additions vs Deletions</p>
              </div>
              <div className="flex items-center gap-3 text-xs">
                <span className="flex items-center gap-1.5">
                  <span className="w-2.5 h-2.5 rounded-sm bg-primary" /> Additions
                </span>
                <span className="flex items-center gap-1.5">
                  <span className="w-2.5 h-2.5 rounded-sm bg-destructive" /> Deletions
                </span>
              </div>
            </CardHeader>
            <CardContent className="p-5 pt-4">
              <div className="h-60">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={days === 0 ? churnData : churnData.slice(-days)} barGap={2}>
                    <CartesianGrid strokeDasharray="3 3" stroke={grid} vertical={false} />
                    <XAxis dataKey="day" {...axisProps} />
                    <YAxis {...axisProps} />
                    <Tooltip content={<ChartTooltip />} />
                    <Bar dataKey="additions" name="Additions" fill={primary} radius={[4, 4, 0, 0]} maxBarSize={24} />
                    <Bar dataKey="deletions" name="Deletions" fill={destructive} radius={[4, 4, 0, 0]} maxBarSize={24} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
        ...
        {contributors.length > 0 && (
          <Card>
            <CardHeader className="p-5 pb-0 flex flex-row items-center justify-between">
              <div>
                <p className="text-sm font-semibold text-foreground">Top Contributors</p>
                <p className="text-xs text-muted-foreground mt-0.5">By commit volume</p>
              </div>
            ...
            <CardContent className="p-5 pt-4 divide-y divide-border/60">
              {contributors.map((c, i) => {
                ...
                return (
                  <div key={c.author} className="flex items-center gap-4 py-3 first:pt-0 last:pb-0">
        ...
        {!isFetching && totalCommits === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm text-muted-foreground max-w-sm">
              Connect your GitHub repos and configure a webhook to start ingesting engineering data.
            </p>
          </div>
        )}
      </div>
