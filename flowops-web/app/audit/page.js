...
export default function AuditPage() {
  ...
  useEffect(() => {
    if (!orgId) return
    setIsFetching(true)
    fetchAuditLogs(orgId, { limit: 50, page, ...(search && { action: search }) })
      .then(({ logs: l, total: t }) => {
        setLogs(l)
        setTotal(t)
      })
      ...
      <div className="p-6 lg:p-8 max-w-[1440px] mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <Shield size={12} />
            <span>
              <span className="font-semibold text-foreground tabular-nums">{total}</span> total events
            </span>
          </div>
        </div>
        ...
        <Card className="overflow-hidden">
          {isFetching ? (
            ...
            <div className="flex flex-col items-center justify-center p-16 text-muted-foreground">
              ...
              <p className="text-sm">
                {search ? 'Try a different filter.' : 'Actions taken in your org will appear here.'}
              </p>
            </div>
          ) : (
                      ...
                      <td className="px-5 py-3.5">
                        {log.user ? (
                          <div className="flex items-center gap-2.5">
                            {log.user.avatarUrl ? (
                              <img src={log.user.avatarUrl} className="w-6 h-6 rounded-full ring-1 ring-border" alt="" />
                            ) : (
                              <div className="w-6 h-6 rounded-full bg-primary/10 flex items-center justify-center text-[10px] font-bold text-primary">
                                {log.user.username?.[0]?.toUpperCase()}
                              </div>
                            )}
                            <span className="text-foreground font-medium">{log.user.username}</span>
                          </div>
                        ) : (
        ...
        {total > 50 && (
          <div className="flex items-center justify-between mt-4">
            <p className="text-xs text-muted-foreground">
              Showing{' '}
              <span className="font-medium text-foreground">{(page - 1) * 50 + 1}</span>–
