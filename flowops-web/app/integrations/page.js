...
const INTEGRATIONS = [
  ...
  },
  {
    ...
    description:
      "Connect Jira to view issues and sprint data alongside your engineering metrics.",
    fields: [
      {
        key: "baseUrl",
        label: "Jira Base URL",
...
export default function IntegrationsPage() {
  ...
  const handleSave = async (type) => {
    ...
    try {
      const saved = await saveIntegration(orgId, {
        type,
        config: forms[type] || {},
      });
      setIntegrations((prev) => {
        ...
        return exists
          ? prev.map((i) => (i.type === type ? saved : i))
          : [...prev, saved];
      });
      setExpanded(null);
    } catch (e) {
    ...
    <Layout>
      <div className="p-4 sm:p-6 lg:p-8 max-w-[1440px] mx-auto">
          ...
          <CardHeader className="p-4 sm:p-5 pb-0">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl border bg-zinc-500/10 border-zinc-500/20 flex items-center justify-center text-xl shrink-0">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-0.5 max-w-md">
                    Connect repos to monitor commits, PRs, and review metrics on
                    the dashboard.
                  </p>
                </div>
              </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-muted-foreground">
                            ...
                            {repo._count &&
                              ` · ${repo._count.commits || 0} commits · ${repo._count.pullRequests || 0} PRs`}
                          </p>
                        </div>
                      </div>
              ...
              <div className="border border-border rounded-xl overflow-hidden bg-card mt-4 animate-in fade-in slide-in-from-top-2 duration-200">
                <div className="p-4 border-b border-border bg-muted/30">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm font-semibold text-foreground">
                      {historyPromptRepo.fullName}
                    </p>
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">
                    How would you like to initialize the dashboard data?
                  </p>
                </div>
                <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  >
                    ...
                    <span className="text-[11px] text-muted-foreground leading-snug">
                      Import all existing commits &amp; PRs so the dashboard
                      shows your complete repo history.
                    </span>
                  </button>
                  <button
                  ...
                  >
                    ...
                    <span className="text-[11px] text-muted-foreground leading-snug">
                      Only track new commits &amp; PRs going forward. The
                      dashboard starts fresh.
                    </span>
                  </button>
                </div>
                <div className="px-4 pb-3 flex justify-end">
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => setHistoryPromptRepo(null)}
                    className="text-xs"
                  >
                    Cancel
                  </Button>
        ...
        {isFetching ? (
          ...
          <div className="space-y-4 max-w-2xl">
            {INTEGRATIONS.map(
              ({ type, label, icon, color, description, fields }) => {
                const connected = getIntegration(type);
                const statusInfo = connected
                  ? STATUS_MAP[connected.status]
                  : null;
            ...
            <div className="relative bg-popover border border-border rounded-2xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground truncate max-w-[220px]">
                      {disconnectModal.repo.fullName ||
                        disconnectModal.repo.name}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-5 space-y-4">
                <p className="text-sm text-muted-foreground">
                  This will stop monitoring and remove commits, PRs, and AI
                  reviews linked to this repo.
                </p>
                ...
                <label className="flex items-start gap-3 cursor-pointer group">
                  <input
                    ...
                    onChange={(e) =>
                      setDisconnectModal((prev) => ({
                        ...prev,
                        purgeData: e.target.checked,
                      }))
                    }
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      Permanently removes sprint health records, AI insights,
                      and generated documentation from the database.
                    </p>
                  </div>
                </label>
              </div>
