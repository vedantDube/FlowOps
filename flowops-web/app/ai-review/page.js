            ...
            {reviewMode === "pr" && (
              <div className="flex gap-3">
                ...
                <Button
                  ...
                  disabled={isTriggering || !prInput.trim()}
                  className="h-10 px-5"
                >
                  {isTriggering ? (
                    <>
          ...
          <div className="lg:col-span-3">
            {!selectedReview ? (
              <Card className="flex flex-col items-center justify-center min-h-[400px] text-center text-muted-foreground">
                ...
                <p className="text-sm">
                  Choose a review from the list to see detailed analysis.
                </p>
              </Card>
            ) : (
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-foreground text-base">
                        {selectedReview.pullRequest?.title ||
                      ...
                      {selectedReview.source === "github" &&
                        selectedReview.filesReviewed && (
                          <p className="text-xs text-muted-foreground mt-1">
                            {selectedReview.filesReviewed.length} file
                            {selectedReview.filesReviewed.length !== 1
                              ? "s"
                              : ""}{" "}
                            reviewed from {selectedReview.repoName}
                          </p>
                        )}
                      <div className="flex items-center gap-3 mt-2">
                        ...
                        {selectedReview.refactorSuggestions?.length > 0 && (
                          <Badge variant="outline" className="text-[10px]">
                            {selectedReview.refactorSuggestions.length}{" "}
                            Refactors
                          </Badge>
                        )}
                      </div>
                  ...
                  {selectedReview.performanceHints?.length > 0 && (
                    ...
                    >
                      {selectedReview.performanceHints.map((p, i) => (
                        <div
                          key={i}
                          className="bg-amber-500/5 border border-amber-500/10 rounded-xl p-4 text-sm text-foreground"
                        >
                          {p.description}
                        </div>
                      ))}
                  ...
                  {selectedReview.antiPatterns?.length > 0 && (
                    ...
                    >
                      {selectedReview.antiPatterns.map((a, i) => (
                        <div
                          key={i}
                          className="bg-orange-500/5 border border-orange-500/10 rounded-xl p-4"
                        >
                  ...
                  {selectedReview.refactorSuggestions?.length > 0 && (
                    ...
                    >
                      {selectedReview.refactorSuggestions.map((r, i) => (
                        <div
                          key={i}
                          className="bg-teal-500/5 border border-teal-500/10 rounded-xl p-4"
                        >
                          <p className="text-sm text-foreground">
                            {r.description}
                          </p>
